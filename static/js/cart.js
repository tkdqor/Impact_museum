...
for(var i = 0 ; i < updatedBtns.length; i++){
    updatedBtns[i].addEventListener('click', function(){    // 클릭했을 때 -> 밑에 입력한 코드가 정의된 함수를 실행시키자!
        var postID = this.dataset.post                      // button의 data attribute를 가져와서 변수 정의
        var action = this.dataset.action
        console.log('postID:',postID , 'action:',action)    // 개발자도구 console에 해당 데이터를 출력
        ...
        if (user === 'AnonymousUser'){                      // 만약, 로그인이 되지 않아 AnonymousUser라면
            ...
            updateUserOrder(postID,action)                  // 로그인이 된 사용자가 맞다면 -> updateUserOrder라는 함수 실행
        }
    })
} 
